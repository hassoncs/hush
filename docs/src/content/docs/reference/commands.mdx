...
hush set "postgres://user:pass@host/db" DATABASE_URL
hush set "sk_live_xxx" STRIPE_KEY -e production
...
1. **Inline value**: `hush set "value" KEY` - value provided directly
...
```
$ hush set "postgres://localhost/mydb" DATABASE_URL
...
<Aside type="tip">
AI agents can use inline values directly: `hush set "value" KEY`. For user-provided secrets, use interactive mode: `hush set KEY`.
</Aside>
