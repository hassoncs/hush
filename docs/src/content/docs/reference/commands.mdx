...
```bash
# Set a secret in shared (.hush.encrypted)
...
# Set a secret in local overrides (.hush.local.encrypted)
...
✓ DATABASE_URL set in .hush.encrypted (45 chars)
```
