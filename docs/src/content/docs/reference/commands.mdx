...
2. In global mode, auto-creates `~/.hush/hush.yaml` and `~/.hush/.sops.yaml` when the global key already exists
3. Decrypts the target file
4. Sets or updates the key
5. Re-encrypts the file
6. Confirms success with character count (no value shown)
...
# Initialize the explicit global store
hush init --global
```
