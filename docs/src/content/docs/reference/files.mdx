...
## Generated Files (Deprecated Workflow)

<Aside type="caution">
These files are only created by the **deprecated** `hush decrypt` command. With `hush run`, secrets stay in memory and these files are never created.
</Aside>

If you use `hush decrypt` (not recommended), these files are created:

### .env.development / .env.production

Generated dotenv files for the current environment.

**Created by:** `hush decrypt` (deprecated)  
**Commit:** No
...
**Created by:** `hush decrypt` for `format: wrangler` targets (deprecated)  
**Commit:** No

### .env.*.json / .env.*.sh / .env.*.yaml

Generated files for JSON, shell, and YAML formats.

**Created by:** `hush decrypt` (deprecated)  
**Commit:** No

<Aside type="tip">
**Migration:** Instead of `hush decrypt && npm start`, use `hush run -- npm start`. Secrets stay encrypted at rest and are only decrypted to memory when needed.
</Aside>
...
# Hush - Generated files (from deprecated `hush decrypt`)
.env.*.json
.env.*.sh
.env.*.yaml
.dev.vars
