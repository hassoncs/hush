...
Hush uses [SOPS](https://github.com/getsops/sops) with [age](https://github.com/FiloSottile/age) encryption. Install them first:
...
<Aside type="tip">
Add `*.encrypted` files to git. With `hush run`, plaintext env files are never created—secrets only exist in memory when your program runs.
</Aside>
