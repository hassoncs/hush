...
> Historical shipped migration note. This page documents an older released transition. It is not the authority for the current `.hush/` repository model or the modern `hush migrate --from v2` flow.
