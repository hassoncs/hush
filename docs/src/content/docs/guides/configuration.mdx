...
Hush is configured through a single \`hush.yaml\` file in your repository root. 

<Aside type="note">
Hush configuration lives entirely in \`hush.yaml\`. There is **no** \`.hush\` directory or hidden configuration folder.
</Aside>
...
<Aside type="note">
For Cloudflare Workers, `push_to` is optional. Targets with `format: wrangler` automatically push to Workers.
</Aside>
...
<Aside type="caution">
Hush warns you if it detects a `.dev.vars` file when running a Wrangler target. The fix is always: delete the file and use `hush run`.
</Aside>
