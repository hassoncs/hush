...
Copy this prompt into Claude Code to install the skill:

```markdown
Install the Hush secrets skill for this project:

1. Create .claude/skills/hush-secrets/examples directories
2. Download skill files from https://github.com/hassoncs/hush repository:
...
From now on, use hush commands instead of reading .env files.
```
...
If you're using Hush in your project, copy the skill folder:

```bash
# From a Hush repository
cp -r .claude/skills/hush-secrets /path/to/your/project/.claude/skills/
```
