...
Avoid `hush decrypt` (deprecated) - it writes plaintext to disk.
```
...
- Avoid `hush decrypt` (deprecated - writes plaintext to disk)
```
...
Safe to do:
- cat .env.encrypted         # Shows encrypted content
- grep KEY .env.encrypted    # Search encrypted files
...
Avoid:
- hush decrypt (deprecated - writes plaintext to disk)
```
